...
SERVICE: EmbeatService | None = None
...
def initialize_service() -> None:
    ...
    try:
        print("Connecting to Embeat Qdrant database...", flush=True)
        SERVICE = EmbeatService()
        print("Embeat database is ready.", flush=True)
    except Exception as exc:
        SERVICE_ERROR = str(exc)
        print(f"Failed to initialize Embeat: {exc}", flush=True)
    finally:
        SERVICE_READY.set()
...
def require_service() -> EmbeatService:
    ...
    raise RuntimeError("Embeat 数据库仍在初始化，请稍后重试")


class RequestHandler(BaseHTTPRequestHandler):
    server_version = "EmbeatUI/1.0"
    ...
    def do_GET(self) -> None:
        ...
        try:
            if parsed.path == "/api/health":
                if SERVICE is not None:
                    self._json(200, {"ready": True, **SERVICE.health()})
                elif SERVICE_ERROR:
                    self._json(503, {"ready": False, "status": "error", "error": SERVICE_ERROR})
                else:
            ...
            if parsed.path == "/api/preferences":
                ...
                self._json(200, require_service().app_db.get_preferences(user["id"]))
                return
            if parsed.path == "/api/config":
                self._require_user()
                self._json(200, CONFIG.public_defaults())
                return
            ...
            if parsed.path == "/api/search":
                ...
                tracks = require_service().search(name, artist, limit)
                require_service().app_db.add_history(
                    user["id"], "search", f"{name} - {artist}".strip(" -"),
                    {"name": name, "artist": artist, "count": len(tracks)}, tracks,
                )
                self._json(200, {"tracks": tracks})
                return
            if parsed.path == "/api/discover/genres":
                query = parse_qs(parsed.query)
                limit = self._bounded_int(query.get("limit", ["100"])[0], 1, 500)
                self._json(200, {"genres": require_service().genres(limit)})
                return
            if parsed.path == "/api/discover/genre":
                query = parse_qs(parsed.query)
                limit = self._bounded_int(query.get("limit", ["50"])[0], 1, 50)
                result = require_service().discover_genre(query.get("genre", [""])[0], limit)
                require_service().app_db.add_history(
                    user["id"], "genre", f"流派：{result.get('genre', '')}",
                    {"genre": result.get("genre"), "discoveryTitle": result.get("genre"), "note": "按热度浏览该流派"},
                    result.get("tracks"),
                )
                self._json(200, result)
                return
            if parsed.path == "/api/discover/weekly":
                query = parse_qs(parsed.query)
                limit = self._bounded_int(query.get("limit", ["50"])[0], 1, 50)
                result = require_service().weekly_discovery(limit)
                require_service().app_db.add_history(
                    user["id"], "weekly", f"每周新发现 {result.get('week', '')}",
                    {"week": result.get("week"), "discoveryTitle": f"每周新发现 · {result.get('week', '')}", "note": result.get("note", "")},
                    result.get("tracks"),
                )
                self._json(200, result)
                return
            if parsed.path in ("/api/netease/status", "/api/netease/config"):
                self._json(200, require_service().platform_status(user["id"], "netease"))
                return
            if parsed.path == "/api/netease/playlists":
                self._json(200, {"playlists": require_service()._credential_client(user["id"], "netease").playlists()})
                return
            if parsed.path in ("/api/kugou/status", "/api/kugou/config"):
                self._json(200, require_service().platform_status(user["id"], "kugou"))
                return
            if parsed.path == "/api/kugou/playlists":
                self._json(200, {"playlists": require_service()._credential_client(user["id"], "kugou").playlists()})
                return
            if parsed.path == "/api/export/status":
                query = parse_qs(parsed.query)
                self._json(200, require_service().export_manager.status(query.get("id", [""])[0], user["id"]))
                return
            if parsed.path == "/api/netease/export/status":
                query = parse_qs(parsed.query)
                self._json(200, require_service().export_manager.status(query.get("id", [""])[0], user["id"]))
                return
            if parsed.path in ("/api/playlist/seeds", "/api/netease/playlist/seeds"):
                query = parse_qs(parsed.query)
                platform = query.get("platform", ["netease"])[0]
                playlist_id = query.get("id", [""])[0]
                max_seeds = self._bounded_int(query.get("max_seeds", ["30"])[0], 1, 30)
                service = require_service()
                self._json(200, service.playlist_seeds(platform, playlist_id, max_seeds, service._credential_client(user["id"], platform)))
                return
            if parsed.path == "/api/history":
                query = parse_qs(parsed.query)
                self._json(200, require_service().app_db.list_history(user["id"], query.get("kind", [""])[0], self._bounded_int(query.get("page", [1])[0], 1, 100000), self._bounded_int(query.get("page_size", [30])[0], 1, 100)))
                return
            if parsed.path == "/api/export/history":
                query = parse_qs(parsed.query)
                self._json(200, require_service().app_db.list_export_logs(user["id"], self._bounded_int(query.get("page", [1])[0], 1, 100000), self._bounded_int(query.get("page_size", [30])[0], 1, 100)))
                return
            self._serve_static(parsed.path)
        except PermissionError as exc:
            self._json(429 if any(word in str(exc) for word in ("频繁", "过多")) else 401, {"error": str(exc)})
        except Exception as exc:
            self._mark_invalid_for_error(parsed.path, exc)
        ...
        try:
            if parsed.path in ("/api/auth/register", "/api/auth/login"):
                ...
                if parsed.path.endswith("/register"):
                    ...
                    if required_invite and not secrets.compare_digest(str(body.get("invite_code") or ""), required_invite):
            ...
            if parsed.path == "/api/auth/logout":
                ...
                return
            user = self._require_user()
            if parsed.path not in ("/api/recommend", "/api/recommend/multi"):
                body = self._read_json()
                service = require_service()
                if parsed.path == "/api/preferences":
                    updates = {key: body[key] for key in ("theme", "accent_hue") if key in body}
                    self._json(200, service.app_db.update_preferences(user["id"], **updates))
                    return
                if parsed.path in ("/api/netease/captcha/send", "/api/kugou/captcha/send"):
                    platform = "netease" if "/netease/" in parsed.path else "kugou"
                    phone = str(body.get("phone") or "")
                    self._check_captcha_rate_limit(user["id"], platform, phone)
                    self._json(200, service.send_phone_captcha(platform, str(body.get("api_url") or ""), str(body.get("proxy_url") or ""), phone, str(body.get("country_code") or "86")))
                    return
                if parsed.path in ("/api/netease/captcha/login", "/api/kugou/captcha/login"):
                    platform = "netease" if "/netease/" in parsed.path else "kugou"
                    self._json(200, service.login_phone_captcha(user["id"], platform, str(body.get("api_url") or ""), str(body.get("proxy_url") or ""), str(body.get("phone") or ""), str(body.get("code") or ""), str(body.get("country_code") or "86")))
                    return
                if parsed.path == "/api/netease/config":
                    if body.get("clear"):
                        service.app_db.clear_credential(user["id"], "netease")
                        self._json(200, {"configured": False})
                    else:
                        self._json(200, service.configure_platform(user["id"], "netease", body))
                    return
                if parsed.path == "/api/kugou/config":
                    if body.get("clear"):
                        service.app_db.clear_credential(user["id"], "kugou")
                        self._json(200, {"configured": False})
                    else:
                        self._json(200, service.configure_platform(user["id"], "kugou", body))
                    return
                if parsed.path == "/api/export/start":
                    clients = {platform: service._credential_client(user["id"], platform) for platform in ("netease", "kugou")}
                    self._json(200, service.export_manager.start(
                        str(body.get("target") or "netease"),
                        body.get("tracks") or [],
                        body.get("netease") or {},
                        body.get("kugou") or {},
                        user_id=user["id"], clients=clients, app_db=service.app_db,
                    ))
                    return
                if parsed.path in ("/api/netease/export", "/api/netease/export/start"):
                    self._json(200, service.export_manager.start("netease", body.get("tracks") or [], {"playlist_id": str(body.get("playlist_id") or "NEW"), "playlist_name": str(body.get("playlist_name") or "")}, {}, user_id=user["id"], clients={"netease": service._credential_client(user["id"], "netease")}, app_db=service.app_db))
                    return
                if parsed.path == "/api/history":
                    kind = str(body.get("kind") or body.get("type") or "search")
                    tracks = body.get("tracks")
                    service.app_db.add_history(user["id"], kind, str(body.get("title") or "未命名记录"), body.get("summary") or body.get("data"), tracks)
                    self._json(200, {"ok": True})
                    return
                self._json(404, {"error": "接口不存在"})
                return
            body = self._read_json()
            if parsed.path == "/api/recommend/multi":
                limit = self._bounded_int(body.get("limit", 50), 1, 50)
                result = require_service().recommend_multi(body.get("track_ids") or [], limit)
                history_title = str(body.get("history_title") or f"{len(result.get('seeds', []))} 首种子电台")
                require_service().app_db.add_history(
                    user["id"], "radio", history_title,
                    {"seeds": result.get("seeds", []), "elapsed_ms": result.get("elapsed_ms", 0)},
                    result.get("tracks"),
                )
                self._json(200, result)
                return
            track_id = str(body.get("track_id") or "")
            limit = self._bounded_int(body.get("limit", 20), 1, 50)
            result = require_service().recommend(track_id, limit)
            seed = result.get("seed") or {}
            require_service().app_db.add_history(
                user["id"], "recommend", f"{seed.get('track_name', '')} - {seed.get('artist_name', '')}".strip(" -"),
                {"seed": seed, "elapsed_ms": result.get("elapsed_ms", 0)}, result.get("tracks"),
            )
            self._json(200, result)
        except PermissionError as exc:
            self._json(429 if any(word in str(exc) for word in ("频繁", "过多")) else 401, {"error": str(exc)})
    ...
    def _serve_static(self, path: str) -> None:
        routes = {"": "index.html", "/": "index.html", "/radio": "radio.html", "/radio/": "radio.html", "/settings": "settings.html", "/settings/": "settings.html"}
        requested = routes.get(path, path.lstrip("/"))
        file_path = (STATIC_DIR / requested).resolve()
        if STATIC_DIR not in file_path.parents and file_path != STATIC_DIR:
            self._json(403, {"error": "禁止访问"})
            return
    ...
    def _read_json(self) -> dict[str, Any]:
        ...
        if self.path.startswith("/api/") and content_type and "application/json" not in content_type:
            raise ValueError("API 请求必须使用 application/json")
        length = int(self.headers.get("Content-Length", "0"))
        if length <= 0:
            return {}
        return json.loads(self.rfile.read(length).decode("utf-8"))

    def _json(self, status: int, payload: Any, cookie: str = "") -> None:
    ...
    def _check_auth_rate_limit(self) -> None:
        ...
        with AUTH_ATTEMPTS_LOCK:
            recent = [stamp for stamp in AUTH_ATTEMPTS.get(ip, []) if stamp > now - 900]
            if len(recent) >= 10:
    ...
    def _check_captcha_rate_limit(self, user_id: int, platform: str, phone: str) -> None:
        now = time.time()
        normalized_phone = re.sub(r"\s+", "", phone)
        key = f"{user_id}:{platform}:{normalized_phone}"
        with CAPTCHA_ATTEMPTS_LOCK:
            recent = [stamp for stamp in CAPTCHA_ATTEMPTS.get(key, []) if stamp > now - 900]
            if recent and recent[-1] > now - 60:
                raise PermissionError("验证码发送过于频繁，请 60 秒后重试")
            if len(recent) >= 5:
                raise PermissionError("验证码发送次数过多，请 15 分钟后重试")
            recent.append(now)
            CAPTCHA_ATTEMPTS[key] = recent

    def _mark_invalid_for_error(self, path: str, error: BaseException) -> None:
        message = str(error).casefold()
        invalid = any(code in message for code in ("cookie 已失效", "cookie 无效", "-462", " 301", " 302", " 152"))
        if not invalid or SERVICE is None:
            return
        platform = "netease" if "/netease/" in path else "kugou" if "/kugou/" in path else ""
        user = self._current_user()
        if platform and user:
            SERVICE.app_db.mark_credential_invalid(user["id"], platform)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Embeat local web UI")
    ...
    if args.host == "0.0.0.0":
        try:
            import socket
            lan_ip = socket.gethostbyname_ex(socket.gethostname())[2]
            for ip in lan_ip:
                if ip.startswith("192.168.") or ip.startswith("10.") or ip.startswith("172."):
                    print(f"局域网访问: http://{ip}:{args.port}", flush=True)
                    break
        except OSError:
            pass
    try:
